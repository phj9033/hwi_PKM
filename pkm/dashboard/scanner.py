...
1. Walks `<root>/data/`, partitioning markdown files into 4 categories:
   captures, chunks, wiki, writing. (M8: `data/style/<style>/<sample>.md`
   is intentionally excluded from the dashboard surface — style samples are
   searchable via `pkm search --scope style` but get no list/detail pages.)
