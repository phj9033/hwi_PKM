...
def test_log_show_json(tmp_path: Path):
    _init(tmp_path)
    runner.invoke(
        app, ["log", "append", "m1", "--type", "t", "--ref", "r", "--root", str(tmp_path)]
    )
...
def test_log_show_filter_type(tmp_path: Path):
    _init(tmp_path)
    runner.invoke(app, ["log", "append", "a", "--type", "x", "--ref", "1", "--root", str(tmp_path)])
    runner.invoke(app, ["log", "append", "b", "--type", "y", "--ref", "2", "--root", str(tmp_path)])
    res = runner.invoke(app, ["log", "show", "--type", "x", "--json", "--root", str(tmp_path)])
    payload = json.loads(res.output)
